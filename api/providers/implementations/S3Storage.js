    ...
    fs.createReadStream(filepath)
      ...
      .on('entry', (entry) => {
        ...
        if (type === 'File') {
          ...
          const uploadPromise = this.client.upload(params).promise();
          entry.pipe(pass);
          ...
          const combinedPromise = Promise.all([uploadPromise, entryEndPromise])
            .then(([uploadData, sha1]) => {
              return {
                ...
                }.s3.amazonaws.com/${encodeURIComponent(caminhoRelativo)}`,
      ...
      .on('close', async () => {
        try {
          ...
          fs.unlink(filepath);
          return JSON.parse(listaArquivos);
        } catch (err) {
          throw new Error('Erro ao fazer upload para o S3.', err);
        }
      })
      .on('error', (err) => {
        throw new Error('Erro ao processar o arquivo.', err);
      });
  }
}
